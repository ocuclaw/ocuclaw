...
| Need | Command / fact |
|---|---|
| Supported host | non-unknown `status.hermesVersion` decides; only when it is `unknown` does `hermes --version` decide; require `>=0.20.0,<0.21.0` (certified baseline `0.20.6`) |
...
| Optional secrets | the same OcuClaw setup flow offers masked Soniox and Even-AI prompts |
...
Hermes' own key, gateway-wide, and inert below Hermes 0.20.5. Prefer the setup
tool, which asks first and writes atomically:
...
Dark theme (deck black, Even G2 lens green), always listed under Settings >
Appearance > Theme; applied only on a plain yes at fresh-install Step 12:
...
`hermes gateway setup` offers only optional Soniox and Even AI credentials for
