...
(``PluginLlmTrustError`` is a ``PermissionError``) → ``policy-denied``.
...
def normalize_candidate_word(raw: Any) -> Optional[Tuple[str, str]]:
    if not isinstance(raw, str):
        return None
    display = raw.strip()
    if not _LETTER_RE.match(display):
        return None
    return display, display.lower()


def validate_candidates(
    items: Any, *, pattern: str, ways: int, max_candidates: int, hidden_keys: Optional[List[str]] = None
) -> Dict[str, Any]:
    ...
    for raw in source:
        ...
        if word is None:
            ...
            continue
        display, key = word
        if key in seen or key in hidden or not pattern_compatible(display, pattern, ways):
            rejected += 1
            continue
        seen.add(key)
        out.append(display)
...
class InputPredictionRpc:
    ...
    def _policy(self) -> Dict[str, Any]:
        ...
        return {
            "allowModelOverride": bool(raw.get("allow_model_override", False)),
            "allowedModels": [m.strip() for m in allowed if isinstance(m, str) and m.strip()],
        }
    ...
    def _authorize(self, model_choice: str, policy: Dict[str, Any]) -> Tuple[bool, Optional[str], str]:
        if not model_choice or model_choice == "default":
            return True, None, ""
        if not policy["allowModelOverride"] or model_choice not in policy["allowedModels"]:
    ...
    async def handle_capabilities(self, params: Any) -> Dict[str, Any]:
        ...
        models = [{"id": "default", "label": "Connection default", "provider": "", "model": "", "isDefault": True}]
        if supported and policy["allowModelOverride"]:
            for ref in policy["allowedModels"]:
                provider, _, model = ref.partition("/") if "/" in ref else ("", "", ref)
                models.append({"id": ref, "label": ref, "provider": provider, "model": model, "isDefault": False})
    ...
    async def _run(self, req: Dict[str, Any], spec: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            try:
                policy, _route_cfg, scoped = await self._read_profile_state(ns, home, want_route=False)
            except Exception as exc:  # noqa: BLE001
            ...
            ok, model, why = self._authorize(req["modelChoice"], policy)
            if not ok:
